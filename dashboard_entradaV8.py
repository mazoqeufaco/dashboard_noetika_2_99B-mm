...
LABEL_COLORS = {"R": "#ff4d4d", "G": "#baff38", "B": "#52b4ff"}
VERTEX_LABEL_FONT = ("Segoe UI", 28, "bold")
VERTEX_LABEL_OFFSET = 22  # deslocamento para fora do triângulo
...
def barycentric(px, py, a, b, c):
    ...
    if abs(denom) < 1e-9:
        return (0.0,0.0,0.0)
    w1 = _area(px,py,x2,y2,x3,y3) / denom    # peso do vértice 'a' (top)
    w2 = _area(px,py,x3,y3,x1,y1) / denom    # peso do vértice 'b' (left)
    w3 = 1.0 - w1 - w2                       # peso do vértice 'c' (right)
    return (w1,w2,w3)
...
class RGBTriangleApp(tk.Tk):
    def __init__(self, img_path, debug=False):
        ...
        self.window_w = max(640, new_w + 40)
        ...
        # RÓTULOS nos vértices (pra fora do triângulo)
        ...
        # Eventos (clique e arrasto)
        self.canvas.bind("<Button-1>", self.on_click)
        self.canvas.bind("<B1-Motion>", self.on_click)  # arrastar com botão 1
    ...
    def _draw_vertex_labels(self):
        # Mapeamento: topo=B (Prazo), esquerda=R (Custo), direita=G (Qualidade)
        (xt, yt) = self.V_top
        (xl, yl) = self.V_left
        (xr, yr) = self.V_right

        # Prazo (azul) – acima do topo
        self.canvas.create_text(
            xt, yt - VERTEX_LABEL_OFFSET,
            text=LABELS["B"], fill=LABEL_COLORS["B"],
            font=VERTEX_LABEL_FONT, anchor="s"
        )
        # Custo (vermelho) – um pouco à esquerda da base esquerda
        self.canvas.create_text(
            xl - VERTEX_LABEL_OFFSET, yl + VERTEX_LABEL_OFFSET,
            text=LABELS["R"], fill=LABEL_COLORS["R"],
            font=VERTEX_LABEL_FONT, anchor="e"
        )
        # Qualidade (verde) – um pouco à direita da base direita
        self.canvas.create_text(
            xr + VERTEX_LABEL_OFFSET, yr + VERTEX_LABEL_OFFSET,
            text=LABELS["G"], fill=LABEL_COLORS["G"],
            font=VERTEX_LABEL_FONT, anchor="w"
        )
    ...
    # ----- Evento de clique/arrasto no triângulo -----
